...
50590     filename = Replace(filename, "<" & tStr & ">", ReplaceForbiddenChars(isf.FirstDocumentTitle), , , vbTextCompare)
50600    End If
50610  End If
50620  tStr = "JobID"
50630  If Preview Then
50640    filename = Replace(filename, "<" & tStr & ">", "'Preview " & tStr & "'", , , vbTextCompare)
50650   Else
50660    If LenB(isf.FirstJobID) > 0 Then
50670     filename = Replace(filename, "<" & tStr & ">", ReplaceForbiddenChars(isf.FirstJobID), , , vbTextCompare)
50680    End If
50690  End If
50700  tStr = "ClientComputer"
50710  If Preview Then
50720    filename = Replace(filename, "<" & tStr & ">", "'Preview " & tStr & "'", , , vbTextCompare)
50730   Else
50740    If LenB(isf.FirstClientComputer) > 0 Then
50750     filename = Replace(filename, "<" & tStr & ">", ReplaceForbiddenChars(Replace$(isf.FirstClientComputer, "\\", "")), , , vbTextCompare)
50760    End If
50770  End If
50780  tStr = "PrinterName"
50790  If Preview Then
50800    filename = Replace(filename, "<" & tStr & ">", "'Preview " & tStr & "'", , , vbTextCompare)
50810   Else
50820    If LenB(isf.FirstPrinterName) > 0 Then
50830      filename = Replace(filename, "<" & tStr & ">", ReplaceForbiddenChars(isf.FirstPrinterName), , , vbTextCompare)
50840    End If
50850  End If
50860  tStr = "SessionID"
50870  If Preview Then
50880    filename = Replace(filename, "<" & tStr & ">", "'Preview " & tStr & "'", , , vbTextCompare)
50890   Else
50900    If LenB(isf.FirstSessionID) > 0 Then
50910     filename = Replace(filename, "<" & tStr & ">", ReplaceForbiddenChars(isf.FirstSessionID), , , vbTextCompare)
50920    End If
50930  End If
50940
50950  If Options.RemoveSpaces = 1 Then
50960   filename = Trim$(filename)
50970  End If
50980  If Len(filename) >= 4 Then
50990   If InStr(2, filename, "\\") > 0 Then
51000    filename = Mid$(filename, 1, 1) & Replace$(Mid(filename, 2), "\\", "\")
51010   End If
51020  End If
51030  GetSubstFilename2 = filename
'---ErrPtnr-OnError-START--- DO NOT MODIFY ! ---
Exit Function
ErrPtnr_OnError:
