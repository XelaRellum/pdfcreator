...
procedure OCAsyncInit(tszPublisher, tszKey, tszSecret, tszLanguage:OCTString; iInitMode:Integer);
begin
...
function OCShouldSkipPage(CurPageID: Integer) : Boolean;
begin
	Result := false;
	if _OCEnabledAndReady() then
...
//   end;
//
procedure OCCurPageChanged(CurPageID: Integer);
begin
...
function OCNextButtonClick(CurPageID: Integer): Boolean;
begin
	Result := true;
...
procedure OCBackButtonClick(CurPageID: Integer);
begin
	if _OCEnabledAndReady() and gl_OC_bUseOfferPage and (CurPageID = gl_OC_objOCOfferPage.ID) then
		_OCDLL_OCRestorePage(gl_OC_objOCOfferPage.Surface.Handle);
end;
...
procedure OCCurStepChanged(CurStep: TSetupStep);
begin
	if _OCEnabledAndReady() then
	begin
