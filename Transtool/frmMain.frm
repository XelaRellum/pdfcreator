...
Private Sub mnFile_Click(Index As Integer)
 Dim Files As Collection, Filename As String
 Select Case Index
  Case 0:
   If OpenFileDialog(Files, , _
    "Languages-INI-Files (*.ini)|*.ini|All Files (*.*)|*.*", "*.ini", App.Path, _
    App.EXEName, OFN_PATHMUSTEXIST, Me.hwnd) > 0 Then
    If Files.Count > 0 Then
     If Dir(Files.item(1)) <> "" And Len(Trim$(Files.item(1))) > 0 Then
      ShowLanguageIniFile Files.item(1)
     End If
    End If
   End If
  Case 1:
   If SaveFileDialog(Filename, , "Languages-INI-Files (*.ini)|*.ini", "*.ini", App.Path, _
    App.EXEName, OFN_OVERWRITEPROMPT + OFN_PATHMUSTEXIST, Me.hwnd) > 0 Then
    If Len(Trim$(Filename)) > 0 Then
     SaveLanguageIniFile Filename
    End If
   End If
  Case 3:
   Unload Me
